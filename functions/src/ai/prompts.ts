...
`,
  ...
  goalRelevanceGeneration: (goalTitle: string) => `
...
`,
  ...
  goalMilestoneGeneration: (
    ...
    let prompt = `You are an expert goal-setting assistant. Your task is to break down a user's goal into 3-5 sequential, measurable milestones.
...
`;
  ...
  taskSuggestionBasic: (
    ...
    let prompt = `You are a productivity coach helping a user turn their goal into practical, actionable tasks.
...
`;
  ...
  taskSuggestionCustom: (
    ...
    let prompt = `You are a productivity assistant. Your job is to help a user generate tasks for their goal, **paying special attention to their custom request.**
...
`;
  ...
  goalFeasibilityCheck: (
    ...
    let prompt = `You are analyzing a user's goal to determine whether it is realistic and achievable.
